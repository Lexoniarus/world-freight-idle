...
class FleetService:
    ...
    def __init__(
        self, store: SqliteStore, catalogue: VehicleCatalogue
    ) -> None:
        self.store = store
    ...
    def purchase(self, model_id: str) -> dict:
        ...
        with self.store.transaction():
            ...
            vehicle = build_vehicle_snapshot(model, uuid.uuid4().hex)
...
def build_vehicle_snapshot(model: VehicleModel, vehicle_id: str) -> dict:
...
def create_starter_vehicle(catalogue: VehicleCatalogue) -> dict:
    ...
    for model in catalogue.list_models():
        if model.id == "iveco_sway_500":
            return build_vehicle_snapshot(model, "truck_01")
    LOGGER.error(
