...
function Add-BlockOnce {
    ...
    $Updated = $Content + $Separator + "`r`n" + $Block.Trim() + "`r`n"
...
'@
Add-BlockOnce "tests/function_test_manifest.py" "app.services.multiplayer_map.player_color" $ManifestBlock
...
'@
Add-BlockOnce "docs/PRODUCT.md" "## Gemeinsamer Live-Verkehr" $ProductBlock
...
'@
Add-BlockOnce "docs/TARGET.md" "## Abnahme – gemeinsamer Live-Verkehr" $TargetBlock
...
'@
Add-BlockOnce "docs/MILESTONES.md" "### Gemeinsamer Live-Verkehr" $MilestoneBlock
...
'@
Add-BlockOnce "docs/ARCHITECTURE.md" "## Read-only Multiplayer-Verkehrsprojektion" $ArchitectureBlock

Write-Host "Multiplayer map metadata patch complete."
